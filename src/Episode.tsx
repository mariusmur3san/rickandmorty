...
export default function Episode() {
    ...
    const charactersIds = episodes?.flatMap((episode) =>
        episode.characters.map((c) => c.split('/').reverse()[0])
    );
    const {
        isLoading,
        data: characters,
    ...
    if (characters) {
        const caracterComponents = characters.map((character) => {
            ...
            return (
                <LinkItem key={id} route={`/character/${id}`}>
                    <CharacterCard id={id} name={name} imageSrc={image} />
                </LinkItem>
            );
        });
        ...
        return (
            <>
                <h2>
                    Episode {episodes?.[0].episode} : {episodes?.[0].name}
                </h2>
                <p>Air date: {episodes?.[0].air_date}</p>
                <h2>Characters:</h2>
                <Scroller items={caracterComponents} estimateSize={140} />
            </>
        );
    }
